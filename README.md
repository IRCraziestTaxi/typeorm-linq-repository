...
## What's New
As of version 1.0.0-alpha.8, the constructor for `RepositoryBase` may accept optional arguments to specify a specific TypeORM connection to get and/or to indicate that the `id` property of an entity is not an auto-generated primary key. The `id` property is assumed to be auto-generated by default and, unless instructed otherwise, is set to `undefined` during creation to aid in auto-generation of the `id` property.
...
To create the repository using a non-default TypeORM connection, you may specify a connection name as the second argument in the `RepositoryBase` constructor.
...
export class EntityRepository extends RepositoryBase<IEntity> implements IEntityRepository {
    public constructor() {
        ...
        super(Entity, "connection-name");
    }
}
```

If an entity has a property named `id` that is not an auto-generated primary key, you must instruct `RepositoryBase` to not set the `id` property to `undefined` in preparation for auto-generation during creation. In order to do that, pass in `false` as the second argument in the `RepositoryBase` constructor.
...
In order to specify both a non-default connection and an entity with a non-auto-generated `id` property, pass in the connection name as the second argument followed by `false` as the third argument.
...
export class EntityRepository extends RepositoryBase<IEntity> implements IEntityRepository {
    public constructor() {
        // Get the repository from a specific connection.
        // This entity has a property named "id" that is not an auto-generated primary key.
        super(Entity, "connection-name", false);
    }
}
```
