...
## What's New
As of version 1.0.0-alpha.6, the constructor for `RepositoryBase` may accept an optional argument of `false` that specifies that the `id` property of the entity is not an auto-generated primary key. The `id` property is assumed to be auto-generated by default and, unless instructed otherwise, is set to `undefined` during creation to aid in auto-generation of the `id` property.
